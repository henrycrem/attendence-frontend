...
useEffect(() => {
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">
                      Monitor organization-wide attendance and sales performance
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card className="bg-red-600 text-white border-2 border-red-700 shadow-lg">
            ...
            <CardContent>
              ...
              <p className="text-sm opacity-90">
                {data.attendanceStats.presentToday} present today
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white text-black border-2 border-red-300 shadow-lg">
            ...
            <CardContent>
              <div className="text-3xl font-bold mb-1 text-red-600">
                {formatPercentage(parseFloat(data.attendanceStats.attendanceRate))}
              </div>
              <p className="text-sm text-gray-700">
                {data.attendanceStats.lateArrivals} late arrivals
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-black text-white border-2 border-gray-800 shadow-lg">
            ...
            <CardContent>
              ...
              <p className="text-sm opacity-80">
                {data.salesStats.totalConversions} conversions
              </p>
            </CardContent>
          </Card>
  ...
  <CardContent>
    ...
    <p className="text-sm text-gray-700">
      {data.salesStats.remainingToTarget.toFixed(1)} to target
    </p>
  </CardContent>
</Card>
        </div>
    ...
    try {
      ...
      if (attendanceResult.success && attendanceResult.data && attendanceResult.data.length > 0) {
        reports.push({
          ...
          headers: ['Employee Name', 'Email', 'Department', 'Date', 'Sign In', 'Sign Out', 'Workplace', 'Status'],
      ...
      if (salesResult.success && salesResult.data && salesResult.data.length > 0) {
        reports.push({
          title: 'Sales Report',
          headers: ['Sales Rep', 'Client Name', 'Phone', 'Email', 'Task Title', 'Task Type', 'Start Time', 'Completed At', 'Converted', 'Actual Revenue', 'Potential Revenue'],
      ...
      if (teamResult.success && teamResult.data && teamResult.data.length > 0) {
        reports.push({
          title: 'Team Report',
          headers: ['Name', 'Completed Tasks', 'Conversions', 'Revenue', 'Avg Revenue per Conversion'],
      ...
      if (reports.length === 0) {
        reports.push({
          ...
          rows: [
            ['Attendance Report', attendanceResult.success ? 'No Data' : 'Error', attendanceResult.success ? 'No attendance records found for selected date range' : attendanceResult.message || 'Error loading data'],
            ['Sales Report', salesResult.success ? 'No Data' : 'Error', salesResult.success ? 'No sales records found for selected date range' : salesResult.message || 'Error loading data'],
            ['Team Report', teamResult.success ? 'No Data' : 'Error', teamResult.success ? 'No team performance data found for selected date range' : teamResult.message || 'Error loading data']
          ],
          dateRange: {
            from: startDate,
            to: endDate
