...
export async function createTaskAction(params: {
  ...
  console.log("createTaskAction params:", params); // Add this to debug
  try {
...
export const getTasksByUser = getTasksByUserAction
...
export async function getTaskByIdAction(id: string) {
  try {
    ...
    console.log("getTaskByIdAction: Response status =", response.status);

    if (response.status === 404) {
      console.log("getTaskByIdAction: Task not found for id =", id);
      return { success: false, message: "Task not found" };
    }

    if (!response.ok) {
      console.error("getTaskByIdAction: Failed to load task, status =", response.status);
      throw new Error("Failed to load task");
    }

    const result = await response.json();
    console.log("getTaskByIdAction: Backend response =", result);

    if (!result.task) {
      console.error("getTaskByIdAction: Missing 'task' in response");
      throw new Error("Invalid response format: missing task field");
    }
