...
import { useDashboard } from "@/contexts/DashboardContext" // ✅ Import context
...
export default function TopBar({ user, error, onMenuClick }: TopBarProps) {
  ...
  const { isRightSidebarCollapsed, toggleRightSidebar } = useDashboard() // ✅ Get toggle state & function
      ...
      <div className="flex items-center justify-between px-3 sm:px-6 py-3 sm:py-4 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
          ...
          >
            ...
            <span className="text-xs sm:text-sm font-medium text-red-900 whitespace-nowrap">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
          ...
          <button
            ...
            title={isRightSidebarCollapsed ? "Open Sidebar" : "Close Sidebar"}
            className="p-2 text-gray-600 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all duration-200 transform hover:scale-105 flex-shrink-0"
